...
function spatialBnB(U, k_sparse, k_rank, mu, lambda;
    compute_lower_bound=solve_perspective_relaxation, epsilon=0.1,
    ...
    upper_bound_sol, upper_bound_obj = iterative_search(U, mu, lambda, k_sparse,
        k_rank, random_restarts=0, zero_indices=init_zero_indices,
        one_indices=init_one_indices, min_improvement=iter_min_improv)

    lower_bound_sol, lower_bound_obj = compute_lower_bound(U, mu,
    ...
    while (global_upper_bound - global_lower_bound) /
        ...
        for (zero_indices, one_indices) in new_index_list
            ...
            upper_bound_sol, upper_bound_obj = iterative_search(U, mu,
                        lambda, k_sparse, k_rank, random_restarts=0,
            ...
            if isTerminal(one_indices, zero_indices, k_sparse, n)
                ...
                lower_bound_sol, lower_bound_obj = compute_lower_bound(U, mu,
                    lambda, k_sparse, k_rank, zero_indices=zero_indices,
